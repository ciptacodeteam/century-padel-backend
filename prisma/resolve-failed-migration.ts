...
async function main() {
  ...
  try {
    ...
    if (migration.length === 0) {
      console.log('✅ Migration not found in database. It can be applied fresh.')
      return
    }
    ...
    `
    console.log(`   Enum types found: ${enumTypes.map(e => e.typname).join(', ')}`)
    ...
    `
    console.log(`   Tournament tables still exist: ${tournamentTables.map(t => t.tablename).join(', ') || 'NONE'}`)
    ...
    `
    console.log(`   New tables exist: ${newTables.map(t => t.tablename).join(', ') || 'NONE'}`)
    ...
    if ((draftBookings[0]?.count || 0n) > 0n || (draftClassBookings[0]?.count || 0n) > 0n) {
      console.log('\n⚠️  WARNING: Found records with DRAFT status!')
      console.log('   You must update these before re-running the migration:')
      console.log('   UPDATE bookings SET status = \'HOLD\' WHERE status = \'DRAFT\';')
      console.log('   UPDATE class_bookings SET status = \'HOLD\' WHERE status = \'DRAFT\';')
    }
